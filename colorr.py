...
class SmallImage:
  ...
  def __load_img(self):
    try:
        ...
        self.error =  'Error(001): No picture or picture is not in correct format.'
...
class DColor:
  ...
  def remove_skin(self):
    im_ycrcb = cv2.cvtColor(self.img, cv2.COLOR_BGR2YCR_CB)
    skin_ycrcb_mint = np.array((0, 133, 77))
    skin_ycrcb_maxt = np.array((255, 173, 127))
    self.noskin_img = cv2.inRange(im_ycrcb, skin_ycrcb_mint, skin_ycrcb_maxt)
  ...
  def remove_skin_bg_and_crop(self):
    self.remove_skin()
    self.remove_bg()
    self.crop()
    return self
  ...
  def crop(self):
    self.cropped_img = self.crop_img(self.img)
    if hasattr(self, 'nobg_img'):
      self.cropped_nobg_img = self.crop_img(self.nobg_img)
    if hasattr(self, 'noskin_img'):
      self.cropped_noskin_img = self.crop_img(self.noskin_img)
    # return self

  def common_area(self):
    if hasattr(self, 'cropped_nobg_img') and hasattr(self, 'cropped_noskin_img'):
      img = self.cropped_img
      bg = self.cropped_nobg_img
      noskin = self.cropped_noskin_img
    else:
      img = self.img
      bg = self.nobg_img
      noskin = self.noskin_img

    pixels = []
    total_pixals = img.shape[0] * img.shape[1]

    # 20% common area in both nobg and noskin
    for r in range(0,img.shape[0]):
      for c in range(0,img.shape[1]):
        if noskin[r][c] == bg[r][c] == 0:
          pixels.append((int(img[r][c][2]),int(img[r][c][1]),int(img[r][c][0])))
    if len(pixels) > total_pixals / 5:
      return pixels

    # remove background, keep skin
    pixels = []
    for r in range(0,img.shape[0]):
      for c in range(0,img.shape[1]):
        if bg[r][c] == 0:
          pixels.append((int(img[r][c][2]),int(img[r][c][1]),int(img[r][c][0])))
    if len(pixels) > total_pixals / 2:
      return pixels

    # remove skin, keep background for style with similar color as background
    pixels = []
    for r in range(0,img.shape[0]):
      for c in range(0,img.shape[1]):
        if noskin[r][c] == 0:
  ...
  def main_colors(self):
    pixels = self.common_area()
    self.clusters = KMean(pixels, 3, 1).get()
    self.main_colors_count = []
...
class KMean:
  ...
  def __init__(self, pixels, k, min_diff):
    points = []
    for count, pixel in enumerate(pixels):
        points.append(KMean.Point(pixel, 3, count))
    self.points = points
    self.k = k
  ...
  def calculate_center(self, points, n):
    ...
    # plen was 0, cas it causes errors
    plen = 1
    for p in points:
        plen += p.ct
        for i in range(n):
            vals[i] += (p.coords[i] * p.ct)
    return KMean.Point([(v / plen) for v in vals], n, 1)

  def get(self):
    clusters = [KMean.Cluster([p], p, p.n) for p in random.sample(self.points, self.k)]

    while 1:
      plists = [[] for i in range(self.k)]
      ...
      for i in range(self.k):
        ...
        new = KMean.Cluster(plists[i], center, old.n)
        clusters[i] = new
